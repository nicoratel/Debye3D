...
class NanoparticleScattering2D:
    ...
    # -----------------------------
    def compute_intensity(self, positions=None, nproc=None, block_size=500):
        ...
        with Pool(processes=nproc) as pool:
            ...
            for res in tqdm(pool.imap(process_block_wrapper, args),
                            total=len(args),
                            desc="Calcul intensité",
                            unit="bloc"):
                results.append(res)
    ...
    def compute_intensity_old(self, positions=None, nproc=None, block_size=500):
        if positions is None:
            positions = self.positions
        if nproc is None:
            nproc = psutil.cpu_count(logical=False) or os.cpu_count() or 1
    ...
    def make_isotropic(self,I_aniso):
        """
        Convertit une image anisotrope en image isotrope par moyenne radiale (version vectorisée).
        
        Paramètre :
        I_aniso : ndarray
            Image 2D anisotrope (numpy array)
        
        Retour :
        I_iso : ndarray
            Image 2D isotrope obtenue par moyenne radiale
        """
        ny, nx = I_aniso.shape
        cy, cx = ny // 2, nx // 2  # centre de l'image

        # Grille de coordonnées
        y, x = np.indices(I_aniso.shape)
        r = np.sqrt((x - cx)**2 + (y - cy)**2)  # distance au centre
        r_int = r.astype(int)  # rayon arrondi à l'entier

        # Somme et compte par rayon
        ...
        # Reconstruire l'image isotrope 2D
